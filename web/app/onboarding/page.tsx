...
const GITHUB_APP_NAME = process.env.NEXT_PUBLIC_GITHUB_APP_NAME ?? "roberthdudiver-gitvault";
...
export default function OnboardingPage() {
  ...
  const handleInstall = () => {
    const returnTo = encodeURIComponent(window.location.origin + "/onboarding/callback");
    window.location.href = `${installUrl}?state=${returnTo}`;
  };

  const handleRefresh = async () => {
    setChecking(true);
    // Reload page — AuthProvider will re-fetch /auth/me on next auth state change
      ...
      <div className="w-full max-w-md text-center">
        ...
        <div className="flex flex-col gap-3">
          <button
            ...
            className="flex items-center justify-center gap-2 rounded-lg bg-zinc-900 dark:bg-zinc-100 px-5 py-2.5 text-sm font-medium text-white dark:text-zinc-900 transition-colors hover:bg-zinc-700 dark:hover:bg-zinc-300"
          >
